#!/usr/bin/env python3
"""Batch HTTP client for Get-Back dashboard server (server-side batching)."""
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python batch_http_client.py <dashboard-url> <backend> <amount>")
        print("\nArguments:")
        print("  dashboard-url   Dashboard server URL (e.g., http://localhost:9093)")
        print("  backend         Backend to target (e.g., mkl-backend-http:9091)")
        print("  amount          Number of requests to send (e.g., 1000)")
        print("\nExample:")
        print("  python batch_http_client.py http://localhost:9093 mkl-backend-http:9091 1000")
        sys.exit(1)
    ...
    # Build request
    api_url = f"{dashboard_url}/api/request/http"
    request_data = {
        "backend": backend,
        "amount": amount
    }

    print(f"Sending {amount} HTTP requests to {backend} via {dashboard_url}...")
